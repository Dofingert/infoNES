...
static int lcd_fb_display_px(WORD color, int x, int y)
{
	...
	unsigned int *pen32;
	pen8 = (unsigned char *)(fb_mem + y * line_width_virt + x * px_width);
	pen16 = (unsigned short *)pen8;
	pen32 = (unsigned int *)pen16;
	//*pen16 = color;
	u_int32_t f_color;
	f_color = ((color & 0x1f) << 3) | ((color & 0x7e0) << 5) | ((color & 0xf800) << 8);
	*pen32 = f_color;
	return 0;
}
...
static int lcd_fb_init()
{
	...
	line_width_virt = var.xres_virtual * px_width;
	screen_width = var.yres * line_width_virt;
	lcd_width = var.xres;
	lcd_height = var.yres;
...
int main(int argc, char **argv)
{
	...
	{
		fgets(buf,16,stdin);
		dwKeyPad1 = (buf[0] & 0xf) | ((buf[1] & 0xf) << 4);
